...
sample_size <- try_sub %>%
  mutate(pft = factor(pft, abbr2pft) %>% forcats::lvls_revalue(pft2abbr)) %>%
  group_by(pft) %>%
...
for (ma in c("mass", "area")) {
  ...
  multi_summary <- results_all %>%
    ...
    cv_pred <- cv_mod %>%
      mutate(
        grid = map(data, modelr::data_grid, sample_size = seq(1, 10000)),
    ...
    relative_ci <- ggplot(cv_dat) +
      ...
      scale_x_continuous(trans = "log10", limits = c(2, NA)) +
      coord_cartesian(ylim = c(0, 5)) +
      xlab("Sample size") +
      ylab("Relative width of 95% confidence interval") +
      scale_color_brewer(type = "qual", palette = 2) +
      guides(color = guide_legend(title = "Model type")) +
      theme_bw()
    relative_ci
    ggsave(sprintf("figures/relative_ci_sample_size_%s.pdf", ma),
           relative_ci, width = 5, height = 4)
}
